...
## 11. 작업 완료 보고 — 카톡 메시지 (필수 습관)

**모든 작업(수정·배포·조사)이 끝나면, 현장 카톡방("로또 신규전산방")에 그대로 붙여넣을 보고
메시지를 마지막 응답에 반드시 포함한다** (사용자 지시 8/4 — "항상 작업하고 나면 카톡 만들어줘").

작성 규칙:
- 수신자는 비개발자(정의현 차장 등 운영진). 코드·함수·마이그레이션 파일명 같은 개발 용어는 빼고
  **화면·기능 이름과 눈에 보이는 변화** 중심으로 쓴다.
- 존댓말, 요청받은 항목별 번호 매김(①②③ 또는 1. 2. 3.), 요청 문구를 짧게 인용해 무엇에 대한
  답인지 분명히 한다.
- 각 항목에 (a) 무엇이 어떻게 바뀌었는지 (b) **지금 반영(배포)된 상태인지 아닌지** (c) 현장에서
  확인·협조가 필요한 것이 있으면 그 방법을 쓴다. 배포 전이면 "배포 후 알려드리겠습니다"로 명시.
- 길이는 카톡 1~2 메시지 분량. 코드블록 없이 일반 텍스트로, 복사-붙여넣기 즉시 전송 가능한 형태.

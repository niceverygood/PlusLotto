-- Emergency rollback of admin_site_scope to the captured 2026-09-09 production RPCs.
-- Apply only together with reverting the frontend site-scope release.
-- This script never deletes members, payments, workforce entries, or other user data.
-- daily_work_count remains available so newly entered counts cannot be lost.
BEGIN;
DROP FUNCTION IF EXISTS public.admin_dashboard(text);
DROP FUNCTION IF EXISTS public.admin_member_facets(text,text);
DROP FUNCTION IF EXISTS public.admin_member_search(text,integer,text);
DROP FUNCTION IF EXISTS public.admin_nav_badges(text);
DROP FUNCTION IF EXISTS public.admin_payment_counts(text);
DROP FUNCTION IF EXISTS public.admin_revenue(text,date,date,text,text);
DROP FUNCTION IF EXISTS public.admin_revenue_calendar(date,text,text);
DROP FUNCTION IF EXISTS public.admin_revenue_day_payments(date,text,text);
DROP FUNCTION IF EXISTS public.admin_stats_snapshot(text,date,date,text);
DROP FUNCTION IF EXISTS public.admin_consult_report(text,text,date,date,text);
DROP FUNCTION IF EXISTS public.admin_revenue_daily_summary(date,date,text);
...
CREATE OR REPLACE FUNCTION public.admin_member_facets(p_assigned_staff_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
...
from base;
$function$;
REVOKE EXECUTE ON FUNCTION public.admin_member_facets(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_member_facets(text) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.admin_member_search(p_term text DEFAULT ''::text, p_limit integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
...
$function$;
REVOKE EXECUTE ON FUNCTION public.admin_member_search(text,integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_member_search(text,integer) TO authenticated, service_role;
...
with filtered as not materialized (
  ...
  where (p_assigned_staff_id is null or m.assigned_staff_id = p_assigned_staff_id)
    ...
    and (not (p_filter ? 'sourceSite') or m.meta->>'source_site' = p_filter->>'sourceSite')
...
$function$;
REVOKE EXECUTE ON FUNCTION public.admin_nav_badges() FROM PUBLIC, anon;
...
$function$;
REVOKE EXECUTE ON FUNCTION public.admin_payment_counts() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_payment_counts() TO authenticated, service_role;
...
CREATE OR REPLACE FUNCTION public.admin_revenue(p_view text, p_from date, p_to date, p_group text DEFAULT 'staff'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
GRANT EXECUTE ON FUNCTION public.admin_revenue(text,date,date,text) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.admin_revenue_calendar(p_month date, p_view text DEFAULT 'real'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
GRANT EXECUTE ON FUNCTION public.admin_revenue_calendar(date,text) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.admin_revenue_day_payments(p_day date, p_view text DEFAULT 'real'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
GRANT EXECUTE ON FUNCTION public.admin_revenue_day_payments(date,text) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.admin_stats_snapshot(p_view text, p_from date, p_to date)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
...
$function$;
REVOKE EXECUTE ON FUNCTION public.admin_stats_snapshot(text,date,date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_stats_snapshot(text,date,date) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.admin_consult_report(p_dimension text, p_period text, p_from date, p_to date)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
...
from grouped;
$function$;
REVOKE EXECUTE ON FUNCTION public.admin_consult_report(text,text,date,date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_consult_report(text,text,date,date) TO authenticated, service_role;

DROP FUNCTION IF EXISTS public.admin_validate_source_site(text);
DROP FUNCTION IF EXISTS public.member_operating_site(public.members);
DROP INDEX IF EXISTS public.members_operating_site_registered_idx;

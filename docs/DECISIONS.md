...
- **★ 새로 드러난 문제 — 덤프에 과거 유료회원이 빠져 있다**: 대응표상 815 유료회원은 **21,809명**인데 덤프는 **13,237명**뿐이다(인포로또도 5,078 vs 4,403). 차이 8,572명은 결제 후 만료된 회원 — **재계약 영업 대상**이다. 받을지 결정이 필요하다(§4-8, 업체 요청서 부록 2번).
...
- **모의 실행으로 검증**: 815 → 13,237명(goldp 9,030 · vip 3,640 · royal 567) / 결제 21,680건 128.9억 / 발송요일 월3,213·화2,804·수2,805·목2,573·금1,841 — **D167 손 검증치와 정확히 일치**한다. 작성 중 `sorted()` 가 요일 미지정(None) 키에서 터지는 것을 모의 실행이 잡아냈다(815 에 1명 있다).
- **일행로또(cplotto) 실측**: 유료회원 **1,014명**(VIP 959 · 로얄 55), 결제 3,032건 21.0억, 상담원 43명 **미배정 0명**, 90일 내 만료 **370명(36%)**. 결제수단이 `siteCredit`(신용카드) 2,219건으로 **815와 달리 PG 중심**이다.
- **⚠️ 일행로또 덤프에 골드플러스가 한 명도 없다.** 대응표상 골드플러스 4,245명인데 덤프는 VIP·로얄뿐이다(결제 이력에는 `goldplus` 879건이 남아 있다). **대응표 5,658명 → 덤프 1,014명(18%)** 으로 815(72%)보다 이탈이 훨씬 크다 — §4-8(과거 유료회원 이관 여부)이 여기서 훨씬 중요해진다.

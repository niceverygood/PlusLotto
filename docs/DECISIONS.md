...
- **조치**: `api/weekly-reco.ts`에서 발급 대상 배열을 유료등급(`PAID_GRADES`) 먼저 오도록 정렬한 뒤 처리하게 했다. 근본적인 "전체 처리시간이 300초를 넘을 수 있다"는 문제 자체를 없애지는 못하지만(그러려면 배치 분할·재개 같은 더 큰 구조 변경이 필요), 다음에 다시 타임아웃이 나더라도 수가 훨씬 적은 유료회원(결제 고객)이 먼저 끝나 있어 잘리는 쪽이 무료회원이 되도록 우선순위를 바꿨다.
- **즉시 조치**: 오늘 조합이 전혀 없던 유료회원 5명(김광춘·이두식·권승주·임동순·서영선)을 확인해 정의현 차장께 명단을 전달, 수동 발급을 요청했다(가입자 개인정보라 이 스크립트에서 직접 발급·문자발송까지 자동 처리하지는 않음).
- **별건 발견**: `weekly_reco_day`가 아예 설정되지 않은 유료등급 회원이 14명 있었다(전병구 포함) — 이 회원들은 발송요일 미설정 시 "자동발급 대상 아님"이라는 기존 설계(§8) 때문에 애초에 자동발급 대상에 든 적이 없다. 코드 버그가 아니라 운영 설정 누락이라 회원별 발송요일 지정이 필요하다는 점을 별도로 안내.
- **검증**: typecheck·build green. 라이브 `logs`·`members.meta.weekly_recos`·`sms_sends`를 직접 대조해 원인을 특정했다. 실제 프로덕션 데이터를 다시 대량으로 발급/재실행하는 방식의 재현은 실사용자에게 중복 발송 위험이 있어 하지 않았다.

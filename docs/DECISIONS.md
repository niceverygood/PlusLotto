...
- **조합문자 번호 표기 변경**: "조합발송문자 내용 수정... 회차 > 번째" 요청을 번호 표기 `[1]` → `1번째`로 해석해 반영했다(`lib/sms.ts recoSmsBody` + `api/weekly-reco.ts formatComboSms` 동일 반영). 회차 표기(`12.35회차`, 스팸필터 회피용 점 표기)는 7/22 확정 사양이라 그대로 유지 — 다른 의도였다면 재요청 바람.

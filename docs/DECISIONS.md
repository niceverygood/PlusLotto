...
### D96. 배분 카운트 불일치·관리자 리스트·약관 SMS 발송 (현장 7/22, 정의현 차장)
- **①자동/수동배분 순서 반전**: "금일 디비 할당량이 수정반영이 안 되는 것 같다"는 문의 확인 중 실제 버그 재발견 — 라이브(`members/supa.ts`) 의 `assignStaff`/`autoAssign`/`resetAssign` 이 전부 **members UPDATE 를 먼저, assignments 로그(=금일 디비 집계 소스)를 나중에** 남기고 있었다. 특히 `autoAssign` 은 회원별로 개별 `await` update 를 순차 실행한 뒤 assignments 를 한 번에 bulk insert 하는 구조라, 루프 중간에 하나라도 실패하면 이미 배정된 회원들의 로그가 통째로 안 남아 "실제로는 배정됐는데 통계에서 완전히 누락"되는 불일치가 생길 수 있었다. 순서를 반전 — assignments INSERT(로그) 를 먼저 실행하고 성공한 뒤에만 members UPDATE 진행. 최악의 경우도 "기록은 있는데 실제 반영 전" 상태라 재시도로 복구 가능. mock(`members/api.ts`)은 단일 동기 콜백(`mutateDb`)이라 부분실패 여지가 없어 그대로 유지.
- **②관리자 리스트 정렬/필터**: `AdminsPage.tsx` 정렬을 역할그룹+이름순 → **로그인 ID 기준 오름차순**으로 통일. 상단에 역할별 필터 탭(전체/최고관리자/관리자/실장/팀장, 각 인원수 표시) 신설 — `Tabs` 컴포넌트 재사용, `canManageStaff` 계층 위임 스코프는 그대로 유지한 뒤 그 안에서 필터링.
...
- **검증**: typecheck·build green. 라이브(Supabase) 브라우저 확인 — 관리자 화면 로그인ID 정렬(`admin01→jjone0001→…`)·역할 필터 탭(전체43·최고관리자1·관리자2·실장13·팀장27) 정상, 설정>사이트 설정에 `terms` 템플릿 카드 및 안내문구 노출.

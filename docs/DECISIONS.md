...
### D121. 자동배분 "동일 상담원에게 계속 배분" 재발 — 커서 방식 폐기, 배정이력 기반 순환으로 교체 (현장 7/30, 정의현 차장)
- **증상**: "자동배정인데, 동일 상담원에게 배분되었습니다" — 첨부된 배정이력이 `자동 안혜련` → `수동 미지정(리셋)` → `자동 안혜련`으로, D115에서 고쳤던 것과 **정확히 같은 증상**이 다시 보고됐다.
- **원인**: D115의 수정(`site_settings.auto_assign_cursor`에 마지막 배정 담당자를 적어두고 다음 호출이 이어받는 방식)은 **두 가지 전제가 모두 충족돼야만** 동작한다 — (1) `20260728163000_auto_assign_cursor.sql` 마이그레이션이 라이브 DB에 적용돼 있을 것, (2) 자동할당 실행자가 `site_settings` 쓰기 권한을 가질 것(`0002_rls.sql settings_write` = admin·manager). 그런데 커서 읽기/쓰기는 실패해도 배분 자체를 막지 않도록 **조용히 폴백**하게 설계돼 있어(의도된 안전장치), 둘 중 하나만 어긋나도 **아무 경고 없이 수정 전 동작("항상 풀의 첫 담당자")으로 되돌아간다**. D115가 mock에서만 검증되고(당시 검증 기록도 typecheck·mock UI 조작뿐) 라이브 컬럼 적용 여부가 확인되지 않은 점, 7/29 D117에서 `promo_slides` 마이그레이션은 라이브 적용을 명시적으로 기록한 반면 이 컬럼은 그런 기록이 없는 점이 이를 뒷받침한다.
- **조치**: 커서(별도 상태)를 아예 없애고, **배정 이력(`assignments`) 자체에서 순번을 도출**하도록 바꿨다(`src/lib/autoAssign.ts` 신규 — mock·supabase 두 경로가 공유하는 순수 함수). 규칙은 한 줄이다: **"마지막으로 배정받은 지 가장 오래된(또는 받은 적 없는) 담당자부터, 한 명씩 돌아가며"**. 방금 배정받은 사람은 자동으로 순번 맨 뒤로 밀리므로 리셋 후 재배분·풀 가감·1건씩 반복 실행 어디서도 직전 담당자로 되돌아가지 않는다. 추가 컬럼도, 설정 쓰기 권한도 필요 없어 **라이브에 SQL을 적용하지 않아도 배포 즉시 동작**한다(자기치유 — 틀어질 상태 자체가 없다). 배치 내 배정 시각은 1ms씩 벌려 기록한다 — 한 배치 전원이 같은 시각이면 다음 호출에서 "오래된 순"이 전부 동률이 돼 순번이 늘 풀 첫 사람부터 다시 시작하기 때문.
- **일부러 택하지 않은 대안(중요)**: 처음에는 "금일 배분디비 건수를 맞추는 균등분배(부하 기준)"로 구현했는데, 실제 화면에서 검증해 보니 **버그 증상과 구분되지 않았다** — 수동배정 등으로 이미 많이 받은 담당자를 건너뛰느라 한 사람에게 연속으로 몰리는 구간이 생기고, 운영진 눈에는 그게 "또 같은 사람"으로 보인다(모의 데이터에서 5회 연속 같은 담당자 재현). 화면 문구도 '라운드로빈'이므로 **건수 균등이 아니라 순환**을 규칙으로 확정했다.
- **알려진 성질(버그 아님)**: ① 자동배분 대상 담당자가 **1명뿐이면** 당연히 매번 같은 사람에게 간다 — 증상이 계속되면 `관리자` 화면에서 '자동배분 대상' 체크가 몇 명에게 켜져 있는지부터 확인해야 한다. ② 같은 인원(예: 6명)을 풀 크기의 배수만큼 반복 배분하면 매번 같은 매핑이 나올 수 있다 — 분배는 균등하며(3/3), 순환 규칙상 자연스러운 결과다. ③ `assignments`는 RLS로 "볼 수 있는 회원" 범위만 읽히므로 팀장 계정이 실행하면 자기 스코프 안에서의 순환이 된다(설정 커서와 달리 권한이 없다고 아예 무력화되지는 않는다).
- **검증**: typecheck·build green. ① 순수 함수 시뮬레이션 6종(리셋→재배분 6회 연속 동일 담당자 0건, 9명 일괄 3/3/3, 4명씩 3회 나눠 실행 시 호출 간 순환 유지 4/4/4, 편중 이력 후 복구, 신규 담당자 우선, 풀 1명). ② 실제 UI(mock, 브라우저 자동조작)로 리포트된 흐름을 그대로 재현 — 회원 1명에 자동할당→담당리셋을 5회 반복해 `담당 박 → 담당 최 → 담당 박 → 담당 최 → 담당 박`으로 교대 확인(수정 전 동일 절차에서는 5회 모두 `담당 박` 고정을 먼저 재현). 6명 일괄 자동할당은 3/3으로 균등, 관리자 화면 '금일 배분디비' 집계도 정상 반영.

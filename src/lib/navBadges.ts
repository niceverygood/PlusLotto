...
import { matchesSiteScope, rpcSourceSite, type SiteScope } from './siteScope'
...
function scopedWaitCount(user: CurrentUser, db: Pick<DbShape, 'members' | 'payments'>, siteScope: SiteScope): number {
  const byId: Record<string, Member> = {}
  for (const m of db.members) byId[m.id] = m
  const wait = db.payments.filter((p) => p.status === 'wait' && matchesSiteScope(byId[p.member_id]?.meta, siteScope))
...
export function useNavBadges(): Partial<Record<NavKey, number>> {
  ...
  const scope = `nav:${siteScope}:${user?.role ?? 'none'}:${user?.id ?? 'anon'}`
  ...
  const badges = useQuery({
    ...
    queryFn: async () => {
      ...
      if (dataSource === 'supabase') {
        const { data, error } = await sb().rpc('admin_nav_badges', { p_source_site: rpcSourceSite(siteScope) })
      ...
      const db = readDb()
      return { payments: scopedWaitCount(user, db, siteScope), support: openInquiryCount(db) }
    },
    staleTime: 30_000,

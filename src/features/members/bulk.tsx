  ...
  return (
    <>
            ...
            <Button
              ...
              onClick={() =>
                autoAssign.mutate(
                  ...
                  {
                    onSuccess: (r) => {
                      // 담당자 풀이 좁아 과거 담당자를 피하지 못한 건이 있으면 알린다(현장 8/7).
                      if (r.repeated > 0) {
                        window.alert(
                          `${r.assignedIds.length}건을 자동배분했습니다.\n` +
                            `그중 ${r.repeated}건은 선택한 담당자 전원이 이미 담당했던 이력이 있어\n` +
                            `과거 담당자를 피하지 못했습니다.\n\n` +
                            `대상 담당자를 더 넓히면 중복을 피할 수 있습니다.`,
                        )
                      }
                      done()
                    },
      ...
      >
        ...
        {/* 동일 담당자 재배분 금지(현장 확정 8/7) — 실행 전에 규칙을 알린다. */}
        <p className="mb-2 rounded-md bg-warning-bg px-3 py-2 text-[11.5px] leading-relaxed text-ink-700">
          선택한 디비는 <b>모두 배분</b>되며, 각 회원은 <b>과거에 담당했던 상담원을 피해서</b> 배정됩니다
          (담당리셋·DB초기화 이전 이력까지 모두 확인). 담당자 풀이 좁아 피할 수 없는 경우에만 중복될 수
          있고, 그 건수는 완료 후 안내됩니다.
        </p>
        {reps.length === 0 ? (

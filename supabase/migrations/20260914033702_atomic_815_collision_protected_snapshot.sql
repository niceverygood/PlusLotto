-- 운영자가 이관 준비 이후 변경한 정상 업무 기록은 보존한다.
-- 기존 SHARE ROW EXCLUSIVE 잠금 안에서 이관 직전/직후 전체 행을 비교하므로,
-- 이관/트리거가 기존 같은 번호의 계약이나 결제를 바꾸면 모두 롤백된다.
-- 적용 시 회원/결제/발송 데이터 변경 없음. 서비스 전용 함수만 강화한다.
...
BEGIN
  ...
  -- 오래된 외부 스냅샷과 달리 현재 트랜잭션이 잠금을 얻은 시점의 운영값을 보호한다.
  -- 기존 전화 인덱스와 같은 좌변을 사용하며 사이트/결제 상태에 관계없이 보존한다.
  ...
  END IF;
  -- 원래 ID는 전화번호가 바뀌어도 확인한다. 예정된 새 회원 이외에 같은 번호의
  -- 행이 부수적으로 생겼다면 그것도 불일치로 처리해 전체 이관을 취소한다.
  ...
  IF v_members_after IS DISTINCT FROM v_members_before
    ...
    RAISE EXCEPTION USING ERRCODE = 'P0001',
      MESSAGE = '815 collision import changed a protected existing contract; transaction rolled back';
  END IF;
  RETURN pg_catalog.jsonb_build_object('batch_id', p_batch_id, 'members', v_members,
    ...
    'protected_members',pg_catalog.jsonb_array_length(v_members_before),
    'protected_payments',pg_catalog.jsonb_array_length(v_payments_before),
    'existing_full_rows_unchanged',true);
END;
$function$
;
...
COMMENT ON FUNCTION public.admin_import_815_collision_batch(text,jsonb,jsonb,integer,integer,bigint) IS
  '서비스 전용 815 충돌 이관. 같은 트랜잭션 잠금 안에서 모든 기존 동일번호 회원과 연결 결제의 전체 행 불변을 검증하며 불일치 시 전체 롤백.';

-- 발급조합 삭제 권한을 실장(leader)까지 확장한다(현장 피드백 7/24, 정의현 차장).
-- 0021(admin_delete_member_reco)의 admin/manager 전용 체크에 leader 를 추가하는 것 외에는 변경 없음.
...
declare
  ...
  v_safe_round text;
begin
  if (select app_role()) is null or (select app_role()) not in ('admin', 'manager', 'leader') then
